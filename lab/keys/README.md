...
`lab/stage-samba-base.sh` reads every `*.pub` file in this directory at
...
`stage-samba-base.sh` will refuse to build the seed. The deployed
appliance would have no SSH path in — only the console wizard's
`[P]ssword` action would work. If that's actually what you want
(console-only deploy), pass `--allow-no-keys` to the stager.
